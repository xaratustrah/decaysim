#!/usr/bin/env python
'''
simulate decay times according to statistical distributions
...
class Simulator:
    def __init__(self, config):
        ...
        logger.info(f'Writing files to {self.settings_output_path}')
    ...
    def create_from_events_boxcar(self, x):
        ...
        for i in range(self.params_n_sim):
            # create a single random number based on an exponentioal distribution
    ...
    def create_from_distribution(self, x):
        ...
        hist = np.histogram(data, bins=int(
            self.params_timestep *  self.params_n_sim_steps / self.params_timestep), range=(0, self.params_timestep *  self.params_n_sim_steps))[0]
        # normalize histo
        return hist/np.max(hist)

    def create_from_events_with_fluctuations(self, x, trial_number, add_empty_shots=False):
        b_arr = np.array([])
        empty_shot_mask = np.random.choice(
            [0, 1], size=self.params_n_sim, p=[self.params_empty_shots_probability, 1 - self.params_empty_shots_probability])
        ...
        for i in range(self.params_n_sim):
            ...
            if self.params_n_decay_steps == 0:
                ...
                b[idx+1:len(x)] = np.random.normal(self.params_mean_bkgnd,
                                                self.params_stdv_bkgnd, len(x) - idx - 1)

            if add_empty_shots and empty_shot_mask[i]:
                b = np.random.normal(self.params_mean_bkgnd, self.params_stdv_bkgnd, len(x))
            ...
            if self.settings_plot_every_event:
                ...
                # comment out title for publication
    ...
    def fit_exponential(self, x, y):
        p = [self.params_mean_ion / 10, self.params_tau_seed / 5, self.params_mean_bkgnd]
    ...
    def fit_gauss_to_result_array(self, xvals, yvals, mu_est, sigma_est, title=''):
        hist = np.histogram(yvals, bins=1000,
                            range=(-2*self.params_tau_seed, 2*self.params_tau_seed))[0]
        ...
        try:
            popt, pcov = curve_fit(Simulator.gaussian_function, xvals, hist, p0=initial_guess)            
    ...
    def fit_and_plot_gaussian(self, xvals, yvals, bins=100, title=""):
        ...
        sigma_initial = (max(bin_centers) - min(bin_centers)) / 4  # Rough width estimate
        initial_guess = [A_initial, mu_initial, sigma_initial]

        try:
            # Fit Gaussian curve
            popt, _ = curve_fit(Simulator.gaussian_function, bin_centers, counts, p0=initial_guess)
            ...
            #plt.xlabel("X values")
            plt.xlabel(f"amp = {popt[0]:.2e}, mean = {popt[1]:.2e}, sigma = {abs(popt[2]):.2e}")
...
# -----------------
    ...
    def start(self):
        ...
        x = np.arange(0, self.params_timestep *  self.params_n_sim_steps, self.params_timestep)  # start, stop, step in seconds
        ...
        for trial_number in tqdm(range(self.params_n_trials)):
            try:
                ...
                y1 = self.create_from_events_with_fluctuations(
                    x,
                    trial_number,
                    add_empty_shots=self.params_empty_shots)
                popt_events, pcov_events = self.fit_exponential(x, y1)
                tau_events_arr[trial_number] = popt_events[1]
                sigma_events_arr[trial_number] = np.sqrt(pcov_events[1,1]) # get error of taus from events
                ...
                #now comes MLE, which is unbinned
                tau_mle_arr[trial_number] = self.get_mle(x)
                
                # format integer to 4 digits
                self.plot_time_and_fit(
                    x, y1, popt_events, title=f'trial-{trial_number:04}_from_events', display_fit=True)
                # self.plot_time_and_fit(
        ...
        # make plots
        xvals = np.arange(-self.params_tau_seed, self.params_tau_seed, 2*self.params_tau_seed/1000)
        
        # ----- MLE
        logger.info('Creating tau_tru from unbinned MLE distribion.')
        #self.fit_gauss_to_result_array(xvals, self.params_tau_seed - tau_mle_arr, 0, 1, title='tau_tru_from_mle')
        self.fit_and_plot_gaussian(xvals, self.params_tau_seed - tau_mle_arr, title = 'tau_tru_from_mle')
        
        # ----- add up
        logger.info('Creating tau_tru distribution from add up spectra.')
        #self.fit_gauss_to_result_array(xvals, self.params_tau_seed - tau_events_arr, 0, 1, title='tau_tru_from_addup')
        popt = self.fit_and_plot_gaussian(xvals, self.params_tau_seed - tau_events_arr, title='tau_tru_from_addup')
        ...
        #self.fit_gauss_to_result_array(xvals, self.params_tau_seed - tau_events_arr, 0, 1, title='sigma_from_addup')
        popt = self.fit_and_plot_gaussian(xvals, sigma_events_arr, title='sigma_from_addup')
        ...
        self.fit_and_plot_gaussian(xvals, sigma_events_arr / sigma_tru_from_addup, title = 'rho_from_addup')
        ...
        # make a scatter plot for check
        logger.info('Creating the scatter plot.')
        fig, axs = plt.subplots()
        axs.step(np.arange(len(tau_events_arr)), self.params_tau_seed -
                tau_events_arr, where='post', label='tau_from_trials')
        # axs.step(np.arange(len(tau_distro_arr)), self.params_tau_seed -
        #         tau_distro_arr, where='post', label='tau_from_distro')
        axs.step(np.arange(len(tau_mle_arr)), self.params_tau_seed -
                tau_mle_arr, where='post', label='tau_from_mle')        
        axs.set(xlabel='No. of trials', ylabel=r'$\tau_{seed} - \tau_{sim}$',
        ...
        if self.simulation_error_flag:
            logger.warning('There were some errors during the simulation. You might need to start again.')    
